...
def make_pc_suffer_and_enter_values(matrix: List[List[int]], rows:int, columns:int) -> List[List[int]]:
    for i in range(rows):
        for j in range(columns):
            input_ok = False
            if helpers.is_black_square(i, j):
                while not input_ok:
                    value = rnd.randint(0, 9999)
                    if helpers.is_even_with_odd_number_of_elements(int(value)):
                        matrix[i][j] = int(value)
                        input_ok=True

            if not helpers.is_black_square(i, j):
                while not input_ok:
                    value = rnd.randint(0, 9999)
                    if helpers.is_odd_with_even_number_of_elements(int(value)):
                            matrix[i][j] = int(value)
                            input_ok=True
    return matrix
